...
So you might think that to speed things up we should only look at one example at a time rather than 100,000 and update the parameters with the gradient from that example. This is called **stochastic gradient descent** *(SGD)*. the **stochastic** is because the gradient steps are noisy and appears random, since improving on one example may not necessarily improve performance over the entire training set, and the model may actually go in the wrong direction. However the idea is on average the model will go in the right direction, and by taking more frequent steps it will get there faster.

However stochastic gradient descent isn't very efficient since we lose out on the benefit of computing multiple examples in parallel in the forward and backward steps. So while we make more steps, one cycle through the entire training set is much slower.

So instead as a compromise between the frequency of steps and speed of cycling through the dataset, we compute the gradient on a subset of the dataset each time. We call this subset a **minibatch** (with the whole dataset referred to as a **batch**) - giving the name **mini-batch** gradient descent. Since the network is learning from only that batch, the gradient is still noisy, but it is averaged over the batch so more stable than stochastic gradient descent. 

One other benefit the noise has is that it means the network doesn't fully converge to, and thus get stuck in, local minima - this means the network can 
...
A typical value of $$ \beta=0.9 $$ (though sometimes at the start of learning we set $$ \beta=0.5 $$ while we "warm up"). 
...
The exponentially-weighted term comes from the $$\beta$$ being raised to higher powers as we go back in time - since $$\beta<1$$ this means we weight the previous time steps less when taking our average.

#### Code:

One improvement we can make to momentum is to correct our gradient update to account for the momentum - using our physics analogy we look at where the ball's momentum would be taking it first, then correct our gradient update to compensate for this. This is called **Nesterov Accelerated Gradient momentum**.
...
Recall in our [Learning by Gradient]() post we briefly touched upon the learning rate $$\alpha$$ and how it determines the size of the step we take - if it is too small learning is slow, whereas if it is too large we overshoot. 

In the context of SGD the updates are noisy so as we get towards the optimum value we want to take smaller and smaller steps to prevent us from overshooting once we are close, since the gradient will not be exactly zero for the minibatch, even if it is zero when averaged across all examples . 

This idea of reducing the learning rate is known as **learning rate decay**. A simple way of reducing the learning rate may be in *steps* - so maybe halve it every 3 *epochs* (cycles through the training set). However, how do we choose these steps? These seem a bit arbitrary, and depend on the dataset. 
...
**Adagrad** measures the frequency by the root-mean-square of the gradient updates - so (with $$\epsilon=10^{-8}$$ to prevent division by zero): 
...
**RMSProp** takes Adagrad and instead of taking the sum of the squared gradients, takes the *exponentially-weighted moving average*, just like we did for momentum to get our average update, except it uses the squared gradients ( here $$\gamma=0.9$$ is a good value).

$$ s_t = \gamma s_{t-1} + (1-\gamma)\nabla x_{t}^2 $$
$$ x_t = x_{t-1} - \frac{\alpha}{\sqrt{\sum_{i=1}^{t-1} (s_t + \epsilon}} \nabla x_{t-1} $$
...
We will add a bias correction term, to account for the fact that $$s$$ and $$v$$ are intially zero, so take time to warm up (for the first few timesteps they'll be biased towards zero).
